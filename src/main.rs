...
fn main() {
    let cfg = FLICamconfig::from_ini(&get_out_dir().join("asicam.ini")).unwrap_or_else(|_| {
        println!(
            "Error reading config file {:#?}, using defaults",
            &get_out_dir().join("asicam.ini").as_os_str()
        );
        let cfg = FLICamconfig::default();
...
impl FLICamconfig {
    fn from_ini(path: &PathBuf) -> Result<FLICamconfig, String> {
        let config = Ini::new().load(path)?;
        let mut cfg = FLICamconfig::default();
